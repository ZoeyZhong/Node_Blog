var formidable = require('formidable');//用于解析表单数据的Node.js模块，特别是文件上传
var db = require("../model/db.js");    //数据库增删改查
var md5 = require("../model/md5.js");  //对用户输入密码进行加密处理
var fs = require("fs");                //文件系统，载入fs模块，模块中所有方法都有同步和异步两种形式
var moment = require('moment');        //moment.js日期处理类库
var MongoClient = require('mongodb').MongoClient, test = require('assert');//assert库是nodejs中的一个工具库， 主要用于测试模块
//首页
...
exports.showRecording = function (req, res, next) {
    if(req.session.login != "1"){
        ...
        res.render("recording");//views/recording.ejs编写页面
    }
};
exports.doRecording = function (req, res, next) {
    
    var form = new formidable.IncomingForm();//创建一个from表单，uploadDir设置上传文件时临时文件存放的位置

    form.parse(req, function (err, fields) {//parse方法解析node.js中request请求中包含的form表单提交的数据
        db.getAllCount("article", function (count) {
            var allCount = count.toString();
            var date = moment(new Date()).format('YYYY-MM-DD HH:mm:ss');//设置时间格式
            //写入数据库
            db.insertOne("article", { //insertOne插入一条数据
...
exports.getArticle = function (req, res, next) {
    var page = req.query.page;//req.query包含在路由中每个查询字符串参数属性的对象。如果没有，默认为{}
